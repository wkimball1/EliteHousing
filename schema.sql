 ...
 */
drop function
  if exists handle_new_user cascade;
...
CREATE POLICY
  "Enable all for authenticated users only" ON "public"."customers" AS PERMISSIVE FOR ALL TO authenticated
WITH
  CHECK (true);
 ...
 */
drop type
  if exists pricing_type cascade;

drop type
  if exists pricing_plan_interval cascade;
...
drop type
  if exists subscription_status cascade;
...
create table
  subscriptions (
    ...
    id text primary key,
    customer_id text references customers not null,
...
alter table
  ...
  CREATE POLICY
  "Enable all for authenticated users only" ON subscriptions AS PERMISSIVE FOR ALL TO authenticated
WITH
  CHECK (true);
