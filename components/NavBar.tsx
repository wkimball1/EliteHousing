...
export default function NavBar({children} : {children: React.ReactNode}) {
  ...
  return (
    ...
    <nav className="lg:hidden z-10 bg-background px-2 w-full max-w-full" >
      <div className="flex justify-center justify-between w-full max-w-full">
        <Link className="flex justify-start" href="/">
          <img src={defaultLogo.src} alt="Company Logo" width={150} height={100} className="lightLogo"/>
          <img src={darkLogo.src} alt="Company Logo" width={150} height={100} className="darkLogo"/>
        </Link>
        <button
    ...
    {/* desktop */}
    <nav
      className="hidden sticky top-0 z-10 lg:block w-full max-w-full px-4 py-2 text-foreground bg-background border rounded-none shadow-md h-max border-background/80 bg-opacity-80 backdrop-blur-2xl backdrop-saturate-200 lg:px-8 lg:py-4">
